...
## 3) démarche CI/CD
- Un pipeline GitHub Actions existait déjà mais était mal placé (`frontend/.github/workflows/`, non détecté par GitHub vu que j'ai merge les repo "frontend", "backend" et "consignes") et ciblait la branche `master` au lieu de `main`. 
Je l'ai déplacé à la racine du dépôt (`.github/workflows/ci-cd.yml`) + corrigé (branche `main`, `working-directory: frontend`).
- Ce pipeline exécute les tests unitaires Angular (`ng test --browsers ChromeHeadless`) puis construit le bundle de production. 
>> Le job backend (tests + build/push image + déploiement AKS) reste à ajouter.
## 4) configuration AKS
